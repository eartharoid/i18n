module.exports = {
	...
	'commands': {
		'message': {
			'create': { 'name': 'Create ticket from message' },
			'pin': {
				'name': 'Pin message',
				'not_pinnable': {
		...
		'slash': {
			...
			'force-close': {
				...
				'confirm_multiple': {
					'description': 'You are about to close **{count}** tickets that have been inactive for more than `{time}`:\n{tickets}\n',
					'title': '❓ Are you sure?',
				},
				'confirmed_multiple': { 'description': 'The channels will be deleted in a few seconds.' },
				'description': 'Forcibly close a ticket',
				'name': 'force-close',
				'no_tickets': {
					'description': 'There are no open tickets that have been inactive for more than `{time}`.',
					'title': '❌ No tickets',
				},
				'not_staff': {
				...
				'options': {
					'category': {
						'description': 'Close all tickets in the specified category (must be used with `time`)',
						'name': 'category',
					},
					'reason': {
		...
		'user': {
			'create': {
				...
				'sent': { 'description': '{user} has been invited to create a ticket in **{category}**.' },
			},
		},
	},
	'dm': {
		...
		'confirm_open': { 'title': 'Do you want to open a ticket with the following topic?' },
	},
	'log': {
		'admin': {
			'changes': 'Changes',
	...
	'menus': {
		'category': { 'placeholder': 'Select a ticket category' },
		'guild': { 'placeholder': 'Select a server' },
	},
	'misc': {
		'blocked': {
		...
		'expired': { 'description': 'You didn\'t respond in time. Please try again.' },
	...
	'ticket': {
		...
		'offline': {
			'description': 'There aren\'t any staff members available at the moment, so it may take longer than usual to get a response.',
		...
		'working_hours': {
			'next': {
				'description': 'We\'ll be back at <t:{timestamp}:F> (<t:{timestamp}:R>), although you may receive a response before then.',
				'title': '🕗 We\'re not working at the moment',
			},
			'today': {
				'description': 'You may receive a response before, but we don\'t start working until <t:{timestamp}:t> today (<t:{timestamp}:R>).',
				'title': '🕗 We\'re not working at the moment',
			},
		},
	},
